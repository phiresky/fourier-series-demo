		...
		return (
			<div>
				<Range
					...
					tipFormatter={(i: number) => (
						<InlineMath math={this.fns[i].tex} />
					)}
				/>
				<JXGBoard
				...
				<p style={{ visibility: fns.length > 1 ? "" : "hidden" }}>
					Displayed in red:{" "}
					<InlineMath
						math={raw`\frac{4}{\pi}(${fns[fns.length - 1].tex})`}
					/>
				</p>
			</div>
		);
		...
		return (
			<div className="container">
				...
				<ul>
					...
					<li>
						it makes sense to examine the components of a speech
						signal
					</li>
				</ul>
				<p>
					A sum of the kind{" "}
					<InlineMath
						math={raw`\frac{1}{2} a_0 + a_1 \cos x + b_1 \sin x + a_2 \cos x + b_2 \sin x + ...`}
					/>{" "}
					is called a trigonometric sum. A benign periodic function{" "}
					<InlineMath>f(x)</InlineMath> with the period{" "}
					<InlineMath>2\pi</InlineMath> can be expressed as a
					trigonometric sum.
				</p>
				<div>
					<h3>Example</h3>
					<InlineMath
						math={raw`f(x) = \mathbf{sign}(x) \text{ in } [-\pi, +\pi]`}
					/>{" "}
					and periodic elsewhere: <img src={exampleImg} />
				...
				<footer>
					<small>
						<a href="https://github.com/phiresky/fourier-series-demo">
							Source on GitHub
						</a>
					</small>
				</footer>
