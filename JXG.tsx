...
export class JXGBoard extends React.Component<
	{ style?: any; options: any },
	{}
> {
	static boardId = 1;
	...
	componentDidMount() {
		this.board = JXG.JSXGraph.initBoard(this.div.id, {
...
@mobxReact.observer
export class FunctionGraph extends React.Component<
	{
...
> {
	...
	componentWillMount() {
		if (!this.context.board)
			throw Error(
				"FunctionGraph must be created within JXGBoard context"
			);
		const { function: fn, min = -20, max = 20, ...options } = this.props;
		this.functionGraph = this.context.board.create(
			"functiongraph",
			[fn, min, max],
			options
		);
